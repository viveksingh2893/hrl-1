...
function App() {
  ...
  const data=[
    {title:'ABOUT',link:["details","team","admin","contact"],item:["HRL DETAILS","TEAM","ADMIN","CONTACT"]}, 
    {title:'CONCEPT',link:["concept","concept","concept"],item:["MITH.AI","DHARA","SKY BRIDGE"]},
    {title:'RESOURCE',link:["blog","gallery","news"],item:["BLOG","GALLERY","NEWS"]},
  ...
  return (
    ...
    <Layout>
    <Header  style={{justifyContent:'space-around',alignItems:'center'}}>
         ...
         {data.map((val,ind)=>{ 
           return(
            ...
            {val.item.map((newVal,index)=>{
              return(
                <Menu.Item
                onClick={()=>navigate(`/${val.link[index]}`,{state:newVal})}
                > {newVal}</Menu.Item>
