...
setVisible0(false)
 ...
 if (props.type=='text') {return (
  ...
  <div style={{display: 'flex' ,border:'1px solid red',alignItems:'center',justifyContent:'space-around',flexDirection:'column',padding:2}}><div style={{display: 'flex' }}>
    {props.data.video?null:<Modal centered={true} closable={false} destroyOnClose={true} visible={visible0} cancelButtonProps={{ style: { display: 'none' } }} onOk={()=>editText(vidurl)}><Input placeholder='Enter embed video URL' onChange={e=>{setVidurl(e.target.value)}}/></Modal>}
    <Paragraph editable={{onChange:editText}} mark={true} >{props.data.video?props.data.video:vidurl}</Paragraph>
